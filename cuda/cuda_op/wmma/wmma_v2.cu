...
__global__ void wmma_v2_naive_aligned(__restrict__ half* A, __restrict__ half* B, 
    ...
    // 1个warp处理64x64的C
    wmma::frament<wmma::accumulator,WMMA_M,WMMA_N,WMMA_K,half> fragC[4][4];
    // fragA处理64x32的A
...
__global__ void wmma_v3_cpAsync_aligned(__restrict__ half* A, __restrict__ half* B,
    ...
    for(int bk=0;bk<ktimes;bk++){
        ...
        for(int i=0;i<ld_a_times;i++){
            ...
            asm volatile(
                ...
                ::"l"(p_smemA_base + ROW_OFFSET(ld_a_smem_m + stride,ld_a_smem_n,BLOCK_SIZE_K)*sizeof(half)),
        ...
        for(int i=0;i<ld_b_times;i++){
            ...
            asm volatile(
                "cp.async.ca.shared.global [%0],[%1],16;    \n"
                :: "l"(p_smemB_base + ROW_OFFSET(ld_b_smem_m + stride,ld_b_smem_n,BLOCK_SIZE_N)*sizeof(half)),
    ...
    int st_c_gmem_m = by*BLOCK_SIZE_M;
    int st_c_gmem_n = bx*BLOCK_SIZE_N;
...
__global__ void wmma_v4_db_aligned(__restrict__ half* A, __restrict__ half* B,
    ...
    {   
        ...
        for(int i=0;i<ld_a_times;i++){
            ...
            :: "l"(ptr_a_smem_base + ROW_OFFSET(ld_a_smem_m + stride,ld_a_smem_n,BLOCK_SIZE_K)*sizeof(half)),
        ...
        for(int i=0;i<ld_b_times;i++){
            int stride = i*ld_b_stride;
            asm volatile("cp.async.ca.shared.global [%0],[%1],16;   \n"
            :: "l"(ptr_b_smem_base + ROW_OFFSET(ld_b_smem_m + stride,ld_b_smem_n,BLOCK_SIZE_N)*sizeof(half)),
               "l"(&B[ld_b_gmem_addr + stride*N]));
        }

        asm volatile("cp.async.commit_group;    \n" ::);
        asm volatile("cp.async_wait_group 0;    \n" ::);
        __syncthreads();
    }
    ...
    for(int bk=1;bk<ktimes;bk++){
        ...
        for(int i=0;i<ld_a_times;i++){
            int stride = i*ld_a_stride;
            asm volatile("cp.async.ca.shared.global [%0],[%1],16;   \n"
            :: "l"(ptr_a_smem_base + (write_idx * ld_a_buffer_stride + ROW_OFFSET(ld_a_smem_m + stride,ld_a_smem_n,BLOCK_SIZE_K)) * sizeof(half)),
               "l"(&A[ld_a_gmem_addr + stride*K]));
        }   
        ...
        for(int i=0;i<ld_b_times;i++){
            int stride = i*ld_b_stride;
            asm volatile("cp.async.ca.shared.global [%0],[%1],16;   \n"
            :: "l"(ptr_b_smem_base + (write_idx * ld_b_buffer_stride + ROW_OFFSET(ld_b_smem_m + stride,ld_b_smem_n,BLOCK_SIZE_N)*sizeof(half))),
               "l"(&B[ld_b_gmem_addr + stride*N]));
        }
        ...
        for(int i=0;i<4;i++){
            #pragma unroll
            for(int j=0;j<4;j++){
                wmma::mma(fragC[i][j],fragA[0][i],fragB[0][j],fragC[i][j]);
                wmma::mma(fragC[i][j],fragA[1][i],fragB[1][j],fragC[i][j]);
            }
        }
        // 算完更新read_idx
        read_idx ^=1;
...
void cpuHGEMM(half* A,half* B,half* C,int M,int N,int K){
    for(int i=0;i<M;i++){
        for(int j=0;j<N;j++){
            half psum = 0.0f;
            for(int k=0;k<K;k++){
                psum += A[OFFSET(i,k,K)] * B[OFFSET(k,j,N)];
            }
        }
        C[OFFSET(i,j,N)] = (half)psum;
    }
}
...
void testError(void (*wmma_func)(half*,half*,half*,int,int,int),
    ...
    wmma_func<<<grid,block>>>(dA,dB,dC,M,N,K);
    checkCudaError(cudaMemcpy(hdC,dC,sizeC,cudaMemcpyDeviceToHost));

    float maxError = 0.0f;
    for(int i=0;i<M;i++){
        for(int j=0;j<N;j++){
            float thisError = fabs((float)hC[OFFSET(i,j,N)] - (float)hdC[OFFSET(i,j,N)]);
...
int main(int argc,char** argv){
    ...
    void (*wmma_v2)(half*,half*,half*,int,int,int) = wmma_v2_naive_aligned<BM,BN,BK,THREAD_PER_BLOCK>;
