...
__global__ void sgemmV1(float* __restrict__ A,float* __restrict__ B,float* __restrict__ C,int M,int N,int K){
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    for(int thread_y=0;thread_y<THREAD_SIZE_M;thread_y+=4){
        FLOAT4(regA[0][thread_y]) = FLOAT4(smemA[0][0][ty*THREAD_SIZE_M + thread_y]);
    }
    #pragma unroll
    for(int thread_x=0;thread_x<THREAD_SIZE_N;thread_x+=4){
        FLOAT4(regB[0][thread_x]) = FLOAT4(smemB[0][0][tx*THREAD_SIZE_N + thread_x]);
    }
    ...
    do{
        ...
        for(int i=0;i<BLOCK_SIZE_K-1;i++){
            ...
            #pragma unroll
            for(int thread_y=0;thread_y<THREAD_SIZE_M;thread_y+=4){
                FLOAT4(regA[(i+1)%2][thread_y]) = FLOAT4(smemA[read_idx][i+1][ty*THREAD_SIZE_M+thread_y]); 
            }
            #pragma unroll
            for(int thread_x=0;thread_x<THREAD_SIZE_N;thread_x+=4){
                FLOAT4(regB[(i+1)%2][thread_x]) = FLOAT4(smemB[read_idx][i+1][tx*THREAD_SIZE_N+thread_x]);
            }
            // 计算
            #pragma unroll
        ...
        #pragma unroll
        for(int thread_y=0;thread_y<THREAD_SIZE_M;thread_y+=4){
            FLOAT4(regA[0][thread_y]) = FLOAT4(smemA[read_idx^1][0][ty*THREAD_SIZE_M+thread_y]); 
        }
        #pragma unroll
        for(int thread_x=0;thread_x<THREAD_SIZE_N;thread_x+=4){
            FLOAT4(regB[0][thread_x]) = FLOAT4(smemB[read_idx^1][0][tx*THREAD_SIZE_N+thread_x]);
        }
        #pragma unroll
        for(int p=0;p<THREAD_SIZE_M;p++){
            #pragma unroll
    ...
    for(int p=0;p<THREAD_SIZE_M;p++){
        #pragma unroll
        for(int q=0;q<THREAD_SIZE_N;q+=4){
            FLOAT4(tmpC[OFFSET( ty*THREAD_SIZE_M + p,
                                tx*THREAD_SIZE_N + q,
                                N)]) = FLOAT4(cAux[p][q]); 
        }
    }   
...
int main(int argc,char** argv){
    ...
    int gpu_id = 1; // 指定使用的GPU设备ID,例如这里使用第一个GPU
    ...
    for(int i=0;i<nRepeats;i++){
        ...
        dim3 grid(n/bn,m/bm);
        sgemmV1<bm,bn,bk,tm,tn><<<grid,block/*,sizeof(float)*smemSize*/>>>(dA,dB,dC,m,n,k);
    }
