...
// 经过计算后的数据,输出的txt为record_gemm_4x4_like_blas_neonV2
#define GEMM_M (1024)
#define GEMM_K (256)
#define GEMM_N (256)
...
// 每次的操作展开4,循环展开4的意思,因为micro_kernel里就是一次处理4个4x4的矩阵(重排后很好取数据,一次做掉k维上4个4x4)
 ...
 */
void gemm_4x4_like_blas_asmV1(int m,int n,int k,float* a,int lda,
    ...
    float* sa,*sb;
    sa = asmV1::cacheMalloc(m*k);
    sb = asmV1::cacheMalloc(k*n);
...
void kernel_4x4_V2(int m,int n,int k,float* a,float* b,float* c,int ldc){
    ...
    for(i=0;i+3<m;i+=4){
        for(j=0;j+3<n;j+=4){
                ...
                "asr x9,%[k],#2 \n"
                "loop:  \n"
                "   prfm pldl1keep,[%[sa],#64] \n"
                "   prfm pldl1keep,[%[sb],#64] \n"
                "   KERNEL4x4  \n"
                "   KERNEL4x4  \n"
                "   KERNEL4x4  \n"
                "   KERNEL4x4  \n"
                ...
                : "cc","memory","x0","x1","x2","x9","v0","v1","v2","v3","v4","v5","v6","v7","v8","v9"
                );

                // 下面一版是仿造openblas的sgemm的4x4写的,但是降点了,变成大概3.5gflops
                // __asm__ volatile(
                // /* 
                //     INIT4x4利用zero register对浮点寄存器(s16就是v16.s[0])清零,对应的会影响到整个向量寄存器
                // */
                // ".macro INIT4x4     \n"
                // "fmov s16,wzr       \n"
                // "fmov s17,s16       \n"
                ...
                // ".endm              \n"
                // /*
                //     KERENEL4x4用于计算一次外积(一列(4x1)乘于一行(1x4)),生成一个4x4的矩阵
                // */
                // ".macro KERNEL4x4           \n"
                // "ld1 {v8.2s,v9.2s},[%[sa]]  \n"
                // "add %[sa],%[sa],#16        \n"
                ...
                // ".endm                      \n"
                // /* 
                //     SAVE4x4用于将计算好的4x4阵存回C阵中,我们原先使用的是Caux计算,所以需要加回原来C阵的东西
                // */
                // ".macro SAVE4x4             \n"
                // "ld1 {v0.2s,v1.2s},[%[sc]]  \n"
                // "fadd v16.2s,v16.2s,v0.2s   \n"
                // "fadd v17.2s,v17.2s,v1.2s   \n"
                // "st1 {v16.2s,v17.2s},[%[sc]]\n"

                // "add x10,%[sc],%[ldc_offset]\n"
                // "ld1 {v8.2s,v9.2s},[x10]    \n"
                // "fadd v20.2s,v20.2s,v8.2s   \n"
                // "fadd v21.2s,v21.2s,v9.2s   \n"
                // "st1 {v20.2s,v21.2s},[x10]  \n"

                // "add x11,x10,%[ldc_offset]  \n"
                // "ld1 {v0.2s,v1.2s},[x11]    \n"
                // "fadd v24.2s,v24.2s,v0.2s   \n"
                // "fadd v25.2s,v25.2s,v1.2s   \n"
                // "st1 {v24.2s,v25.2s},[x11]  \n"

                // "add x12,x11,%[ldc_offset]  \n"
                // "ld1 {v8.2s,v9.2s},[x12]    \n"
                // "fadd v28.2s,v28.2s,v8.2s   \n"
                // "fadd v29.2s,v29.2s,v9.2s   \n"
                // "st1 {v28.2s,v29.2s},[x12]  \n"
                // ".endm              \n"
                // // 这一部分将会调用上面的宏函数,并且loop它
                // "INIT4x4            \n"
                // "asr x9,%[k],#2     \n"
                // "loop:              \n"
                // "   prfm pldl1keep,[%[sa],#64]  \n"
                ...
                // :[k] "r"(k),
                //  [ldc_offset] "r"(ldc_offset)
                // :"cc","memory","v0","v1","v8","v9","v16","v17",
                // "v20","v21","v24","v25","v28","v29","x9","x10","x11","x12"
                // );
        #else
