...
*/
void packA(int m,int k,float* a,int lda,float* sa);
// B阵打包
/*
 ...
 */
void gemm_4x4_like_blas_neon(int m,int n,int k,float* a,int lda,
    ...
    for(ms=0;ms<m;ms+=GEMM_M){  // 粗略切个M,感觉像是A充满L2Cache?
        ...
        for(ks=0;ks<k;ks+=min_k){   // 切K,给我的感觉是每次都很均匀
            ...
            // B的第一次打包
            packB(min_k,min_n,b+ks*ldb,ldb,sb);
            ...
            for(mms=ms; mms<ms+min_m; mms+=min_mm){
                ...
                // 依次打包A
                packA(min_mm,min_k,a+mms*lda+ks,lda,sa+(mms-ms)*min_k);
                // 计算此时的C
                kernel_4x4(min_mm,min_n,min_k,sa+(mms-ms)*min_k,sb,c+mms*ldc,ldc);
            }
            ...
            for(ns=min_n;ns<n;ns+=min_n){
                ...
                packB(min_k,min_n,b+ks*ldb+ns,ldb,sb);
...
void packA(int m,int k,float* from,int lda,float* to){
...
void packB(int k,int n,float* from,int ldb,float* to){
    float* b = from;
    float* packedB;
    float tmp1,tmp2,tmp3,tmp4,
          tmp5,tmp6,tmp7,tmp8,
